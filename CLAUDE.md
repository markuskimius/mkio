...
- **Config endpoint** — `GET /config/{path}` serves files from `config_dir`. Requesting `*.json` first looks for a `*.toml` file, parses it, and returns JSON; if no `.toml` exists, falls back to a literal `.json` file. Other extensions are served as-is. Subdirectories supported. Path traversal blocked. Only active when `config_dir` is set in config.
