...
- **Subscribe protocol** — All subscribe messages require a `"protocol"` field (`"subpub"`, `"stream"`, or `"query"`) matching the service's configured protocol; the server rejects mismatches. SubPub requires `topic` (primary key value) and always sends a single-row snapshot + updates. Query supports `snapshot`/`updates` booleans and optional `filter` expressions with `filterable` config. Stream requires `ref` for cursor-based reconnection. All subscribe types support optional `fields` list and `subid`.
